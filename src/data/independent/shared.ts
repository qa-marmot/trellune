...
export interface IndependentLessonSeed {
	readonly day: number;
...
export interface IndependentLesson {
	...
	return Object.freeze(
		source.split(';').map((pair, index) => {
			...
			if (separator <= 0 || separator === pair.length - 1) {
				throw new Error(`Invalid authored curriculum item: ${pair}`);
			}
			return Object.freeze({
				id: `${prefix}-${index + 1}`,
...
export function buildIndependentUnit(
	startDay: number,
	seeds: readonly IndependentLessonSeed[],
): readonly IndependentLesson[] {
	if (seeds.length !== 15)
		throw new Error(`Independent unit starting Day ${startDay} must have 15 days.`);
	return Object.freeze(
		seeds.map((seed, index) => {
			const expectedDay = startDay + index;
			if (seed.day !== expectedDay) {
				throw new Error(`Independent unit sequence is invalid at Day ${expectedDay}.`);
			}
			...
			if (vocabulary.length < 4 || vocabulary.length > 6) {
				throw new Error(`Day ${seed.day} must author 4-6 vocabulary items.`);
			}
