...
export interface B2ChallengeLessonSeed {
	readonly day: number;
...
export interface B2ChallengeLesson {
	...
	return Object.freeze(
		source.split(';').map((pair, index) => {
			...
			if (separator <= 0 || separator === pair.length - 1) {
				throw new Error(`Invalid authored B2 Challenge item: ${pair}`);
			}
			return Object.freeze({
				id: `${prefix}-${index + 1}`,
...
export function buildB2ChallengeUnit(
	startDay: number,
	endDay: number,
	seeds: readonly B2ChallengeLessonSeed[],
): readonly B2ChallengeLesson[] {
	const expectedLength = endDay - startDay + 1;
	if (seeds.length !== expectedLength) {
		throw new Error(
			`B2 Challenge Unit Day ${startDay}-${endDay} must have ${expectedLength} days.`,
		);
	}
	return Object.freeze(
		seeds.map((seed, index) => {
			...
			if (seed.day !== expectedDay) {
				throw new Error(`B2 Challenge unit sequence is invalid at Day ${expectedDay}.`);
			}
			const vocabulary = parseItems(seed.vocabulary, `d${seed.day}-v`);
			const phrases = parseItems(seed.phrases, `d${seed.day}-p`);
			if (vocabulary.length < 2 || vocabulary.length > 4) {
				throw new Error(`Day ${seed.day} must author 2-4 vocabulary items.`);
			}
			if (phrases.length < 1 || phrases.length > 3) {
				throw new Error(`Day ${seed.day} must author 1-3 phrases.`);
			}
			if (
				seed.skillTargets.length < 3 ||
