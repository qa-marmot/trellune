...
1. `supportLanguage: ja`では05-session-schema.jsonのバージョン1.0、
   `supportLanguage: en`では08-session-schema-1.1.jsonのバージョン1.1に準拠。
...
バージョン1.1は`summary`、`evaluation.comment`、`meaning`、`note`、
`explanation`という言語中立fieldを使います。英語本文を`summaryJa`等へ入れません。
Trelluneは厳格検証後に既存永続化modelへ正規化します。バージョン1.0は今後も受理します。
