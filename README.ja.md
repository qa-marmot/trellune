...
- **ローカル優先・オフライン対応:** 学習データはIndexedDBに保存され、取得済みの主要画面は
  オフラインでも利用できます。
- **学習を省略しない継続支援:** 大量の必須復習は順序を保って一時停止・再開でき、未完了の
  練習は進捗や同期データを作らない端末内限定draftから復元できます。
- **今の実力から始められる:** 経験者はStageの区切り（Day 1 / 91 / 181 / 271）から開始
  できます。それより前のDayは「未完了」のままで、進捗・復習・連続記録を補完しません。
  ベースラインの結果で開始地点が自動変更されることはなく、placementや認定でもありません。
...
Todayでは、希望学習時間と現在Stageの実際のCore最小・推奨時間を区別して表示します。
希望時間を短くしても、必須Coreや復習負債を減らしたことにはしません。
...
アプリのUIと365日分の学習支援は **日本語** と **English** に対応しています。端末の表示言語に
合う支援カタログを使いますが、学習データ、安定したカリキュラムID、同期、バックアップは
変わりません。過去に自分で書いた・取り込んだ文章は、保存時の言語のまま残ります。対応範囲と
ロケール追加方法は [Localization](docs/LOCALIZATION.md) を参照してください。
...
技術スタックは React、TypeScript、Vite、Dexie / IndexedDB、Hono、Cloudflare
Workers / D1、Zod、PWA / Workbox、Vitest、Playwright、axe です。
