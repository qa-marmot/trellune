...
The browser working database is IndexedDB schema version 5. It stores normalized profile/settings, progress, sessions, learning/acquisition/review events, cards, mistakes, grammar, assessments, outbox, conflicts, sync state and metadata. IndexedDB remains the only source of learning truth. `localStorage` is read by the one-time legacy migration and by a separate, bounded practice-draft cache; those drafts are language-, day-, learner-, and prompt-scoped, expire after 30 days, never count as Core evidence, and never enter sync or backup. Each user-visible success follows an awaited IndexedDB transaction, and `BroadcastChannel` invalidates read models across tabs. Confirmed device deletion clears normalized data, snapshots and legacy queue state while writing deletion/bootstrap markers in one IndexedDB transaction, then removes the legacy local-storage value. Startup and every synchronization commit recheck the durable marker inside their write transaction, so a blocked legacy-storage removal or response already in flight cannot silently rehydrate the just-deleted profile from D1.
...
An onboarding `entryDay` may be one of the four Stage boundaries: Day 1, 91, 181, or 271. It is a sequencing lower bound, not learning evidence or a certified placement result. The calendar searches for incomplete Core only from that boundary, while every earlier day remains explicitly uncompleted. No progress, review, acquisition, session, assessment, or streak record is synthesized. Existing Dexie v5, sync v1, and backup v2 profiles that predate this additive field normalize to Day 1. D1 migration `0014` adds the same defaulted boundary to `learners` without rewriting any learning table.
...
Required reviews are frozen into a deterministic batch before grading starts. Each grade and the updated batch commit atomically, so a large set can be paused and resumed without losing events or silently changing its order. Recovery progress is derived from that durable batch. Partial Recovery never completes the review step, advances the curriculum day, or increments a Core streak; the final required review preserves the existing Core-completion rule.

The learner's preferred daily budget is a planning preference, not a promise that Core fits within it. Today compares that preference with the current Stage's manifest-owned minimum and recommended ranges. The manifest remains the only workload authority.
