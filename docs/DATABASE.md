...
The current forward-only chain ends at `0014_experienced_learner_entry.sql`. Migration 0013 additively backfills neutral support-language columns while retaining the legacy `*_ja` columns and values for SESSION_JSON 1.0 and rollback readers. Migration 0014 adds `learners.entry_day` with a Day 1 default and the closed Stage-boundary set `1 / 91 / 181 / 271`. Neither migration changes sync v1, backup v2, or Dexie v5.
...
| ------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| `learners`                | Pseudonymous learner keyed by a SHA-256-derived subject; `entry_day` is a Stage boundary, never completion proof.  |
