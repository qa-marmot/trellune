...
Bootstrap also returns server `activeTotalDays`. The browser compares it with bundle availability 180 and supported maximum 540 before applying entities, then stores the last compatible value in existing metadata. Offline startup reuses that value and falls back to availability 180 only when no server value has ever been stored. A newer server curriculum than the installed bundle fails closed through the existing startup error UI; no partial hydration, outbox rewrite or protocol-version change occurs.
