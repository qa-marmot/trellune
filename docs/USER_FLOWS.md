...
flowchart TD
  ...
  B --> C["Stage開始地点・タイムゾーン・開始日・学習目安を入力"]
  C --> D["入力内容を確認"]
  D -->|修正| C
  D -->|保存| E["プロフィール作成"]
  E --> F{"ベースラインを実施?"}
...
- 経験者はDay 1 / 91 / 181 / 271から自己選択できる。前のDayは「未完了」のままで、Core、復習、獲得項目、セッション、連続記録を補完しない。
- ベースラインは現在地の記録であり、公式なplacementやCEFR認定ではない。開始地点はベースライン得点から自動変更しない。
